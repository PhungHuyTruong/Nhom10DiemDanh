...
namespace API.Controllers
{
    ...
    public class BoMonCoSoController : ControllerBase
    {
        ...
        public async Task<ActionResult<BoMonCoSoViewModel>> CreateBoMonCoSo(BoMonCoSoViewModel model)
        {
            ...
            if (!model.IdCoSo.HasValue || model.IdCoSo == Guid.Empty)
            {
                ...
                foreach (var coSo in coSos)
                {
                    ...
                    // Cập nhật thông tin cơ sở trong bảng QuanLyBoMons
                    var quanLyBoMon = await _context.QuanLyBoMons.FindAsync(model.IdBoMon);
                    if (quanLyBoMon != null)
                    {
                        quanLyBoMon.CoSoHoatDong = coSo.TenCoSo;
            ...
            else
            {
                ...
                if (quanLyBoMon != null)
                {
                    var coSo = await _context.CoSos.FindAsync(model.IdCoSo);
                    if (coSo != null)
                    {
                        quanLyBoMon.CoSoHoatDong = coSo.TenCoSo;
                        quanLyBoMon.NgayCapNhat = DateTime.Now;
        ...
        public async Task<IActionResult> UpdateBoMonCoSo(Guid id, BoMonCoSoViewModel model)
        {
            ...
            // Cập nhật thông tin trong bảng BoMonCoSos
